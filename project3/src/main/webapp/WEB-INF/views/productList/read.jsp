	...
	<div class="container">
			...
			<div class="info">
				...
				<div>
					...
					<input style="display: none" type="text" name="uplike" value="${vo.uplike}">
					<input style="display: none" type="text" name="plcount" value="${vo.plcount}">
					<input style="display: none" type="text" name="uid" value="${uid}">
					
			 		<c:if test="${vo.uplike==0}"> 
					<button type="submit" id="dib">🤍</button>
			 		</c:if> 
			 		<c:if test="${vo.uplike==1}"> 
					<button type="button" id="cancle" >❤️</button> 
					</c:if> 
				</div>
			</form>
...
<script>
	...
	//찜 취소
	$(frm).on("click","#cancle",function(e){
		e.preventDefault();
		plcount;
		uplike;
		var pmodel=$(frm.pmodel).val();
